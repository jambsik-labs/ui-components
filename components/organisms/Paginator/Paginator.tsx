...
export const Paginator = ({ total, onClickPage }: PaginatorProps) => {
    const getNumberOfPages = () => Math.floor(total / Pagination.DefaultLimit) + Pagination.offset;
    ...
    const onClickPageHandler = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>, page: number) => {
    ...
    return (
        <div sx={paginatorRootStyles}>
            current page: {currentPage} - items per page: {Pagination.DefaultLimit} ~{paginatorItems}
        </div>
    );
};
